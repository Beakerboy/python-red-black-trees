from __future__ import annotations
from abc import ABC, abstractmethod
from typing import Any, Literal

class NodeBase(ABC):
    def __init__(self) -> None:
        self.parent: NodeBase = NULL_NODE
        self.left: NodeBase = NULL_NODE
        self.right: NodeBase = NULL_NODE
        self._red: bool = True

    @property
    def color(self) -> Literal["red", "black"]:
        return "red" if self._red else "black"

    @color.setter
    def color(self, value: Literal["red", "black"]) -> None:
        if value not in ("red", "black"):
            raise ValueError("Color must be 'red' or 'black'")
        self._red = (value == "red")

    def is_null(self) -> bool:
        return False

    def depth(self) -> int:
        if self.parent.is_null():
            return 0
        return self.parent.depth() + 1

    # Comparison operators should be abstract if they depend on data
    @abstractmethod
    def __lt__(self, other: Any) -> bool: ...
    
    @abstractmethod
    def __eq__(self, other: Any) -> bool: ...

    # Python can derive these if you use functools.total_ordering
    def __le__(self, other: Any) -> bool: return self < other or self == other
    def __gt__(self, other: Any) -> bool: return not self <= other
    def __ge__(self, other: Any) -> bool: return not self < other
    def __ne__(self, other: Any) -> bool: return not self == other


class NullNode(NodeBase):
    def __init__(self) -> None:
        # Don't call super().__init__ to avoid infinite recursion
        self._red = False 
        self.parent = self # Standard for sentinel nodes, but be careful with depth()

    def is_null(self) -> bool:
        return True

    def depth(self) -> int:
        return -1

    def __lt__(self, other: Any) -> bool: return False
    def __eq__(self, other: Any) -> bool: return isinstance(other, NullNode)
    def __repr__(self) -> str: return "NullNode"

# Single sentinel instance (standard pattern)
NULL_NODE = NullNode()
